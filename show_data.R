SERVER_show_data <- function(id, location_id, userID){
  moduleServer(id, function(input, output, session){
    ...
    car_detections <- eventReactive(input$date, {
      query <- str_glue("SELECT * FROM car_detections WHERE location_id = {location_id()} AND date_trunc('day', timestamp) = '{dates_with_data()$day[1]}';")
    ...
    output$scatterplot <- renderGirafe({
      ...
      girafe(ggobj = gg, width_svg=11, height_svg=3) %>% girafe_options(opts_selection(type="single"))
    ...
    output$spectrum <- renderPlot({
      ...
      showNotification(str_glue("point {selected_dot()} selected"))
      ...
      byte_index <- dbGetQuery(content, str_glue("SELECT * from bin_index WHERE location_id = {location_id()} AND timestamp >= '{start_time}' AND timestamp <= '{end_time}';")) %>% tibble
